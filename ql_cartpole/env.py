...
class Env():
    ...
    def __init__(self, env_name='CartPole-v0'):
        ...
        self.cart_velocity_bins = pd.cut([-0.21, 0.21], bins=10, labels=False, retbins=True)[1][1:-1]
        self.pole_angle_bins = pd.cut([-1,1], bins=10, labels=False, retbins=True)[1][1:-1]
    ...
    def get_condensed_state(self, observation):
        ...
        first = np.digitize(x=observation[0], bins=cart_position_bins)
        second = np.digitize(x=observation[1], bins=pole_angle_bins)
        third = np.digitize(x=observation[2], bins=cart_velocity_bins)
        fourth = np.digitize(x=observation[3], bins=pole_velocity_bins)

        state = first * 1000 + second * 1000 + third * 10 + fourth
        return state
